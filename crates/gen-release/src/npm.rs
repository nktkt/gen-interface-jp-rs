...
mod tests {
    ...
    impl TempDir {
        fn new(label: &str) -> Self {
            ...
            let nanos = std::time::SystemTime::now()
                ...
                .map(|d| d.as_nanos())
                .unwrap_or(0);
            path.push(format!("gen-release-npm-{label}-{nanos}-{}", std::process::id()));
    ...
    fn write_npm_package_writes_valid_json() {
        ...
        write_npm_package(tmp.path(), "1.2.3", "owner/repo")
            .expect("write_npm_package");

        let body = std::fs::read_to_string(tmp.path().join("package.json"))
    ...
    fn require_webfont_package_errors_when_dir_missing() {
        ...
        let err = require_webfont_package(&nonexistent)
            .expect_err("should error for missing source");
        let msg = format!("{err}");
        assert!(
